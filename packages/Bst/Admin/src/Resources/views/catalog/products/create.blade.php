...
@section('content')
    <div class="content">
        <form method="POST" action="{{ route('admin.catalog.product.store') }}" >
            ...
            <div class="page-content">
                ...
                <accordian title="{{ __('admin::app.catalog.products.general') }}" :active="true">
                    <div slot="body">
                        ...
                        <div class="control-group" :class="[errors.has('old-sku') ? 'has-error' : '']">
                            <label for="old-sku" class="">Old Sku</label>

                            <input type="text" v-validate="{ required: true, regex: /^[a-zA-Z0-9]+(?:-[a-zA-Z0-9]+)*$/ }" class="control" id="old_sku" name="old_sku" value="{{ request()->input('old-sku') ?: old('old-sku') }}" data-vv-as=""/>

                            <span class="control-error" v-if="errors.has('old-sku')">@{{ errors.first('old-sku') }}</span>
                        </div>
