...
class Cart
{
     ...
     * @throws Exception
     */
    public function addProduct($productId, $data, $selected = null)
    {
        
        $options = DB::table('product_options')->where('product_id', $productId)->get();
      
        if(count($options) > 0 && empty($selected)){
            $opt = new Options();
            $opt->getOptions((object) $productId);
        }
        else
        {
