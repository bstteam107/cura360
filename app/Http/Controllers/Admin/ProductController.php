    ...
    public function update(ProductForm $request, $id)
    {
		...
		if($countoptionold > 0){
			for($a=1;$a<=$countoptionold;$a++){
			    ...
			    $optionoldDis = request()->input('option-display-order-'.$a);
				$optionolddata = array(
						"product_id" =>$id, 
						"title" =>$optionoldTitle, 
						"type" =>$optionoldType, 
						"require" =>$optionoldReq,
						"display_order" =>$optionoldDis
				);
		...
		for($i=1;$i<=$countoption;$i++){
			    ...
			    $optionDis = request()->input('option-display-ordernew-'.$i);
    ...
    public function copy(int $productId)
    {
        ...
        $copiedProduct = $this->productRepository->copy($originalProduct);
           ...
           if ($copiedProduct instanceof Product && $copiedProduct->id) {
			$options = \DB::table('product_options')->where('product_id', '=', $originalProduct->id)->get();
		...
		foreach($options as $key => $value){
				...
				$option_id = DB::table('product_options')->insertGetId($optiondata);
				$optionItems = \DB::table('product_options_item')->where('product_id', '=', $originalProduct->id)->where('option_id', '=', $value->id)->get();
			foreach($optionItems as $keys => $val){
				$optionitemdata = array(
						"product_id" =>$copiedProduct->id, 
						"option_id" =>$option_id, 
