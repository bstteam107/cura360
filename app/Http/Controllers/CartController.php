...
class CartController extends Controller
{
     ...
     public function add($id)
    {
        ...
        try {
            $select_option  = request()->input('customoptions');
            Cart::deactivateCurrentCartIfBuyNowIsActive();
			 $result = Cart::addProduct($id, request()->all(), $select_option);
			    $cart_id = $result->id;
				$count = request()->input('customoptions');
               
			  if($count > 0){
				
				$options  = request()->input('selectedOption'); 
				
				if(!empty($options)){
				$option_total =0 ;
				foreach($options as $key){
					if(!empty($key)){
					$optionItem = \DB::table('product_options_item')->where('id', '=', $key)->first();
					$option = \DB::table('product_options')->where('id', '=', $optionItem->option_id)->first();
					
					$optionitemdata = array(
						"cart_id" =>$cart_id, 
						"product_id"=>$id,
						"opt_id"=>$optionItem->option_id,
						"option_id" =>$key, 
						"option" =>$option->title, 
						"option_item" =>$optionItem->itemtitle, 
						"price" =>$optionItem->itemprice,
						"sku" =>$optionItem->itemsku
					);
					$option_total = $option_total + $optionItem->itemprice;
					$existoption = DB::table('cart_item_options')->where('cart_id', '=', $cart_id)->where('opt_id', '=', $optionItem->option_id)->get();
					if($existoption->count() > 0){
					DB::table('cart_item_options')->where(['cart_id'=>$cart_id,'opt_id'=>$optionItem->option_id])->limit(1)->update($optionitemdata);					
					}
					else{
					DB::table('cart_item_options')->insertGetId($optionitemdata);
					}
					}
				}
			  }
			  else{
				  session()->flash('warning', 'Please select required options!!');
				  return redirect()->back();
			  }	
			  for($i =0; $i <= $count;$i++){
				 $optionschk  = request()->input('selectedchkOption_'.$i); 
				 if($optionschk){
				$option_totals =0 ;
				foreach($optionschk as $key){
					if(!empty($key)){
					$optionItem = \DB::table('product_options_item')->where('id', '=', $key)->first();
					$option = \DB::table('product_options')->where('id', '=', $optionItem->option_id)->first();
					...
					$option_totals = $option_totals + $optionItem->itemprice;
					$existoption = DB::table('cart_item_options')->where('cart_id', '=', $cart_id)->where('opt_id', '=', $optionItem->option_id)->where('option_id', '=', $key)->get();
					if($existoption->count() > 0){
					DB::table('cart_item_options')->where(['cart_id'=>$cart_id,'opt_id'=>$optionItem->option_id])->limit(1)->update($optionitemdata);					
					}
					else{
    ...
    {
       $cart = \DB::table('cart_items')->where('id', '=', $itemId)->first();
	   $result = Cart::removeItem($itemId);

        if ($result) {
			
			DB::table('cart_item_options')->where('cart_id',$cart->cart_id)->where('product_id',$cart->product_id)->delete();
