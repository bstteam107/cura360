...
<form role="search" <?php echo $twentytwenty_aria_label; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped above. ?> method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	<label for="<?php echo esc_attr( $twentytwenty_unique_id ); ?>">
		...
		<input type="search" id="<?php echo esc_attr( $twentytwenty_unique_id ); ?>" class="search-field" placeholder="<?php echo esc_attr_x( 'Search Articles&hellip;', 'placeholder', 'twentytwenty' ); ?>" value="<?php echo get_search_query(); ?>" name="s" />
	</label>
	<button type="submit" class="search-submit"><?php //echo esc_attr_x( 'Search', 'submit button', 'twentytwenty' ); ?><img width="20" height="20" src="<?php echo esc_url( home_url( '/' ) ); ?>wp-content/uploads/2022/05/search.png" class="custom-logo" alt="Blog – Cura360"></button>
</form>
